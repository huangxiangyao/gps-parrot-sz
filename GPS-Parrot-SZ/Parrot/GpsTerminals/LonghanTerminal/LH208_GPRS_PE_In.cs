...
namespace Parrot.Models.Longhan
{
    public class LH208_GPRS_PE_In
    {
        ...
        public event ImageReceivedEventHandler RecivImgEven;
        
        public event TaxiOprationInfoReceivedEventHandler RecivTaxiOprationInfo;

        // Methods
        public LH208_GPRS_PE_In()
        {
        ...
        private void GetMobileImg(string _ID, byte[] body, ref MdtWrapper mobileInfo)
        {
            try
            {
                ...
                string s = Convert.ToBase64String(body, 13, num - 10);
                ...
                byte num5 = body[11];
                byte num6 = body[12];
                this.RecivEven(_ID, 0x65, "接收图片数据", "第：" + num2.ToString() + "路摄像头，图片序号：" + num3.ToString() + "，总数据包：" + num6.ToString() + "，数据包序号：" + num4.ToString(),  mobileInfo);
                lock (ImgHashList)
                {
                    ...
                    if (flag)
                    {
                        ...
                        this.RecivEven(_ID, 2, "接收图片数据", "接收第：" + num2.ToString() + "路摄像头，图片序号：" + num3.ToString() + "，总共：" + num6.ToString() + "包数据已接收完成！",  mobileInfo);
                        if (this.RecivImgEven != null)
                        {
                            long ticks = DateTime.UtcNow.Ticks;
                            for (int j = 0; j < temp.ImageTempList.Count; j++)
                            {
                                this.RecivImgEven(_ID, temp.CCD_ID, ticks, num6, j, temp.ImageTempList[j].ToString(), imgStatu, temp.FilmDateTime,  mobileInfo);
                                if (j >= (num6 - 1))
                                {
                                    temp.ImageTempList.Clear();
                                    temp.ImageID = 0xff;
            ...
            switch (body[2])
            {
                ...
                case 0x84:
                    {
                        ...
                        if (body[9] == 0xe7)
                        {
                            ...
                            else if (body[10] == 0x70)
                            {
                                ...
                                this.RecivAlarmEven(_ID, 1, str2, str3, mobileInfo.LastAV, mobileInfo.LastX, mobileInfo.LastY, mobileInfo.LastV, mobileInfo.LastF.ToString(), DateTime.Now.ToString(),  mobileInfo);
                            }
                            str = "";
                            ...
                            this.RecivEven(_ID, 200, "终端信息", str,  mobileInfo);
                        }
                        else
                        {
                            str = Encoding.Default.GetString(body, 9, body[4] - 6);
                            this.RecivEven(_ID, 3, "终端信息", str,  mobileInfo);
                        }
                        goto Label_03E2;
                    }
                case 0x85:
                ...
                case 0x91:
                    if (!(((body[9] == 0xff) & (body[10] == 2)) & (body[12] == 0x30)))
                    {
                        if (body[9] == 4)
                        {
                            string str4 = Encoding.Default.GetString(body, 10, 13);
                            this.RecivEven(_ID, 600, "抢答订单", str4,  mobileInfo);
                        }
                        if (body[9] == 5)
                        {
                            string str5 = Encoding.Default.GetString(body, 10, 13);
                            this.RecivEven(_ID, 600, "取消订单", str5,  mobileInfo);
                        }
                        if (body[9] == 6)
                        {
                            string str6 = Encoding.Default.GetString(body, 10, 13);
                            this.RecivEven(_ID, 600, "完成订单", str6,  mobileInfo);
                        }
                    }
                    else
                    {
                ...
                case 0x9b:
                    this.ReturnEnclosure_Bus(_ID, body, ref mobileInfo);
                    return;
                ...
                case 0xb3:
                    {
                        ...
                        this.RecivEven(_ID, 11, "主机工作状态", s,  mobileInfo);
                        goto Label_03E2;
                    }
                case 240:
                case 0xff:
        ...
        private void PassPoint(string _ID, byte[] body, ref MdtWrapper mobileInfo)
        {
            ...
            this.RecivAlarmEven(_ID, 1, "区域报警", str, mobileInfo.LastAV, mobileInfo.LastX, mobileInfo.LastY, mobileInfo.LastV, mobileInfo.LastF.ToString(), DateTime.Now.ToString(),  mobileInfo);
        }
        ...
        private void ReturnEnclosure_Bus(string _ID, byte[] data, ref MdtWrapper mobileInfo)
        {
            string str = string.Empty + "总共围栏个数：" + data[10].ToString() + "、所查询的围栏内容：、\r\n";
            double num = 0.0;
            double num2 = 0.0;
            double num3 = 0.0;
            try
            {
                ...
                {
                    num = double.Parse(data[(i * 0x12) + 12].ToString("X2").Remove(0, 1) + data[(i * 0x12) + 13].ToString("X2") + data[(i * 0x12) + 14].ToString("X2")) / 60000.0;
                    num2 = double.Parse(data[(i * 0x12) + 11].ToString("X2").Remove(0, 1) + data[(i * 0x12) + 12].ToString("X2").Substring(0, 1));
                    num3 = num + num2;
                    str = str + " 小纬=" + num3.ToString("00.00000");
                    num = double.Parse(data[(i * 0x12) + 0x10].ToString("X2").Remove(0, 1) + data[(i * 0x12) + 0x11].ToString("X2") + data[(i * 0x12) + 0x12].ToString("X2")) / 60000.0;
                    num2 = double.Parse(data[(i * 0x12) + 15].ToString("X2") + data[(i * 0x12) + 0x10].ToString("X2").Substring(0, 1));
                    num3 = num + num2;
                    str = str + " 小经=" + num3.ToString("00.00000");
                    num = double.Parse(data[(i * 0x12) + 20].ToString("X2").Remove(0, 1) + data[(i * 0x12) + 0x15].ToString("X2") + data[(i * 0x12) + 0x16].ToString("X2")) / 60000.0;
                    num2 = double.Parse(data[(i * 0x12) + 0x13].ToString("X2").Remove(0, 1) + data[(i * 0x12) + 20].ToString("X2").Substring(0, 1));
                    num3 = num + num2;
                    str = str + " 大纬=" + num3.ToString("00.00000");
                    num = double.Parse(data[(i * 0x12) + 0x18].ToString("X2").Remove(0, 1) + data[(i * 0x12) + 0x19].ToString("X2") + data[(i * 0x12) + 0x1a].ToString("X2")) / 60000.0;
                    num2 = double.Parse(data[(i * 0x12) + 0x17].ToString("X2") + data[(i * 0x12) + 0x18].ToString("X2").Substring(0, 1));
                    str = ((str + " 大经=" + ((num + num2)).ToString("00.00000")) + "、围栏号：" + data[(i * 0x12) + 0x1b].ToString()) + "、围栏报警方式：" + data[(i * 0x12) + 0x1c].ToString() + "\r\n";
                }
            }
            catch (Exception exception)
            {
        ...
        private void Taxi_Run_Check(string _ID, byte[] _body, ref MdtWrapper mobileInfo)
        {
            ...
            this.RecivEven(_ID, 2, "出租车营运报告", str3,  mobileInfo);
            if (this.RecivTaxiOprationInfo != null)
            {
                this.RecivTaxiOprationInfo(mobileInfo.SysID.ToString(), payType, taxiID, getInDateTime, getOutDateTime, waitTime, mileage, allroundPrice, priceOfGetOut, freeMileage);
            }
            if (payType == "1")
            {
            }
        ...
        private void UpdateLine_V(string _ID, byte[] body, ref MdtWrapper mobileInfo)
        {
            ...
            this.RecivEven(_ID, 2, "下载节点数据", str,  mobileInfo);
        }
        ...
        private void UpdateMobile_V(string _ID, byte[] body, ref MdtWrapper mobileInfo)
        {
            ...
            if (body[9] == 1)
            {
                ...
                temp[(5 + num5) - 1] = 13;
                this.RecivGpsDataReturn(_ID, Convert.ToBase64String(this.ReturnCmdByte, 0, 10),  mobileInfo, 0);
            }
        }

        private string Vehicle_SetStatu(string _ID, byte A, byte B, byte C, byte D, byte E, byte F, byte G, byte H, byte I, ref MdtWrapper mobileInfo)
        {
            string str = "";
            if ((A & 0x80) == 0x80)
            {
            ...
            string[] strArray = new string[] { "定时发送间隔：", ((B * 0x100) + C).ToString(), "秒、停车报警检测时间：", D.ToString(), "分钟、" };
            int num2 = H * 30;
            return (((string.Concat(strArray) + "超速门限：" + E.ToString("0") + "公里、") + "电子围栏设置：" + F.ToString() + "个、") + "定时发送图片的时间：" + num2.ToString() + "秒");
        }
